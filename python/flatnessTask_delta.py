...
def flatnessTask_delta(raft_id, infiles, dtype='OGP', pickle_file=None):
    ...
    for infile in infiles:
        ...
        datestr.append(pieces[2])
        loc = pieces[3].find('C')
        temperature.append(pieces[3][0:loc])

    print(stdev)
    print(temperature)
    print(datestr)

    # Find the 'best' (least standard deviation, after clipping) scans at 19C and
    # -100C
    temp = np.array(temperature).astype(float)
    loc = np.where((temp == 19))[0]
    std = np.array(stdev)
    pos = np.where(std[loc] == np.min(std[loc]))[0]
    locWarm = loc[pos[0]]
    loc = np.where((temp == -100))[0]
    pos = np.where(std[loc] == np.min(std[loc]))[0]
    locCold = loc[pos[0]]
    ...
    #
    outfile = '%s_flatness_delta_residuals_%s.txt' % (raft_id, pieces[2])
    ...
    metData.plot.save('%s_flatness_delta_hist_%s.png' % (raft_id, pieces[2]))
    #
    # Box and whisker plot of residual heights
    #
    raftDataDelta.resids_boxplot()
    metData.plot.save('%s_flatness_delta_boxplot_%s.png' % (raft_id, pieces[2]))
    #
    # Quantile table
    #
    raftDataDelta.quantile_table(outfile='%s_flatness_delta_quantile_table_%s.txt'
                          % (raft_id, pieces[2]))
    #
    # Surface plots
    #
